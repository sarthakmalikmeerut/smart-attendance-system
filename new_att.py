...
# Add a global variable to store attendance data
...
def mark_attendance(name):
    ...
    if hour not in attendance[name]:
        ...
        if (name, date_string, str(hour)) not in existing_entries:
            with open(file_name, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([name, date_string, str(hour)])
                print(f"Attendance marked for {name} at {hour}:00")

                # Update the global attendance_data variable
                entry = {'Name': name, 'Date': date_string, 'Timestamp': str(hour)}
                attendance_data.append(entry)

                # Emit a socket event to notify the frontend about the new entry
                socketio.emit('attendance_update', entry)
        else:
...
def index():
    path = 'training_images'
    classNames = [os.path.splitext(cl)[0] for cl in os.listdir(path)]
    return render_template('index.html', classNames=classNames, attendance_data=attendance_data)
